...
import { getSleepSchedules } from 'apiSdk/sleep-schedules';
import { SleepScheduleInterface } from 'interfaces/sleep-schedule';

import { SweetspotListPage as ShowCaseList } from 'pages/sweetspots';
...
function DashboardPage() {
  ...
  const babyStat = useSWR<PaginatedInterface<BabyInterface>>(
    () => `/babies`,
    () => getBabies({ limit: 1, offset: 0 }),
  );

  const sleepScheduleStat = useSWR<PaginatedInterface<SleepScheduleInterface>>(
    () => `/sleep-schedules`,
    () => getSleepSchedules({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Babies',
      result: babyStat?.data?.totalCount,
      isLoading: babyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Sleep Schedules',
      result: sleepScheduleStat?.data?.totalCount,
      isLoading: sleepScheduleStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`sweetspots`} passHref legacyBehavior>
                  <Button
