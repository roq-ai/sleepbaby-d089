...
import { getSweetspotById } from 'apiSdk/sweetspots';
import { SweetspotInterface } from 'interfaces/sweetspot';

function SweetspotViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('sweetspot', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/sweetspots/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  ...
                  text={
                    data?.ideal_sleep_window
                      ? format(parseISO(data?.ideal_sleep_window as unknown as string), 'dd-MM-yyyy')
                      : ''
                  }
                />

                <FormListItem
                  label="Ideal Nap Time"
                  text={
                    data?.ideal_nap_time
                      ? format(parseISO(data?.ideal_nap_time as unknown as string), 'dd-MM-yyyy')
                      : ''
                  }
                />

                <FormListItem
                  label="Ideal Night Sleep Time"
                  text={
                    data?.ideal_night_sleep_time
                      ? format(parseISO(data?.ideal_night_sleep_time as unknown as string), 'dd-MM-yyyy')
                      : ''
                  }
                />
                ...
                {hasAccess('baby', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Baby"
                    text={
                      <Link as={NextLink} href={`/babies/view/${data?.baby?.id}`}>
                        {data?.baby?.name}
                      </Link>
                    }
                  />
