...
import { getBabyById } from 'apiSdk/babies';
import { BabyInterface } from 'interfaces/baby';
import { ReportListPage } from 'pages/reports';
import { SleepScheduleListPage } from 'pages/sleep-schedules';
import { SweetspotListPage } from 'pages/sweetspots';

function BabyViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<BabyInterface>(
    () => (id ? `/babies/${id}` : null),
    () =>
      getBabyById(id, {
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('baby', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/babies/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Nap Times" text={data?.nap_times} />
